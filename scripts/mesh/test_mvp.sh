...
native_staking=$(meshd q wasm list-contract-by-code 3 --output json --node tcp://127.0.0.1:26654 | jq -r '.contracts[0]' )
...
gov_addr=$(meshd q auth --node tcp://127.0.0.1:26654  module-account gov -o json | jq ".account.base_account.address")
...
meshd tx gov submit-proposal ./scripts/mesh/update_params.json --from test1 --keyring-backend test --home=$HOME/.meshd/chain2 --node tcp://127.0.0.1:26654 --chain-id chain-2 -y 

sleep 7

meshd tx gov vote 1 yes --from val1 --keyring-backend test --home=$HOME/.meshd/chain2 --chain-id chain-2 --node tcp://127.0.0.1:26654 -y

sleep 30

# stake from provider(chain2) '{"bond":{}}'=======bond============
meshd tx wasm execute $vault '{"bond":{"amount":{"amount": "10000", "denom":"stake"}}}'  --from test1 --home=$HOME/.meshd/chain2  --chain-id chain-2 --keyring-backend test --node tcp://127.0.0.1:26654 --fees 1stake -y --gas 15406929
...
meshd q wasm state smart $vault "$account_query" --node tcp://127.0.0.1:26654
